resource "oci_containerengine_cluster" "oke_mlflow_cluster" {
...
resource "oci_containerengine_node_pool" "mlflow_node_pool" {
  cluster_id         = var.create_new_oke_cluster ? oci_containerengine_cluster.oke_mlflow_cluster[0].id : var.existing_oke_cluster_id
  compartment_id     = var.compartment_ocid
  kubernetes_version = var.kubernetes_version
  name               = var.mlflow_node_pool_name
  node_shape         = var.mlflow_node_pool_shape
  ...
  node_config_details {
    ...
    size = var.mlflow_node_pool_size
  }
  ...
  node_source_details {
    ...
    image_id = element([
      ...
      if(
        length(regexall("GPU", var.mlflow_node_pool_shape)) > 0 ? length(regexall("Oracle-Linux-${var.nodepool_image_version}-Gen2-GPU-20[0-9]*.*-OKE-${substr(var.kubernetes_version, 1, -1)}", source.source_name)) > 0 :
  ...
  initial_node_labels {
    key   = "name"
    value = var.mlflow_node_pool_name
  }
}
